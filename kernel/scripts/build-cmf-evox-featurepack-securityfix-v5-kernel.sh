...
# Rebuild the CMF Phone 1 Evolution X 11.10 FeaturePack security-fix v5 kernel.
# The same kernel payload is packaged against the official GApps and Vanilla
# boot images because both variants use the same header-v4 kernel layout.
# Usage: ./build-cmf-evox-featurepack-securityfix-v5-kernel.sh [work-directory]

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
KERNEL_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
BUILD_ROOT=${1:-"$PWD/cmf-evox-featurepack-33252-securityfix-build"}
...
COMMON_PATCH_FILE="$KERNEL_DIR/patches/CMF-Phone-1-EvolutionX-11.10-FeaturePack-KSUNext-SUSFS-SUExecFix-v3-source.patch"
KSU_PATCH_FILE="$KERNEL_DIR/patches/CMF-Phone-1-EvolutionX-11.10-KSUNext-33252-SUSFS-SUExecFix-v4.patch"
CONFIG_FILE="$KERNEL_DIR/config/CMF-Phone-1-EvolutionX-11.10-FeaturePack-SecurityFix-v5.config"
...
grep -q '^CONFIG_BPF_UNPRIV_DEFAULT_OFF=y$' "$OUT_DIR/.config"
if grep -q '^CONFIG_BBG_BLOCK_RECOVERY=y$' "$OUT_DIR/.config"; then
    echo "Recovery write blocking must stay disabled for OrangeFox ROM flashing." >&2
    exit 1
fi
...
echo "Built KernelSU Next 33252 security-fix v5 image: $BUILD_ROOT/Image.lz4"
